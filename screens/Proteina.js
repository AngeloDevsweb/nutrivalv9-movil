...
const Proteina = () => {
    ...
    useEffect(() => {
        const getLista = async()=>{
            try {
                const querySnapshot = await getDocs(collection(db, 'proteinas'))
                const docs = []
    ...
    return (
        ...
        <View style={styles.padre}>
          ...
          >
            <Text style={styles.textoImagen}>PROTEINAS</Text>
          </ImageBackground>
        </View>
        {/* fin del encabezado */}
        <Text style={styles.title}>Por 100gr. de alimento se obtiene un (N°) de proteinas en gramos</Text>
